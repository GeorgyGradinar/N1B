  ...
  <div class="page">
    <div class="page-inner section">
      ...
      <article class="article">
        ...
        <h1 class="article-title">{{ t('servicesTree.sites.sub.webapp') }}</h1>
          ...
          <table class="comparison-table">
            <thead>
              <tr>
                ...
                <th scope="col">{{ comparison.platform.label }}</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--pros" role="list">
                    <li v-for="(item, i) in comparison.platform.pros" :key="'p-p-' + i">{{ item }}</li>
                  </ul>
                </td>
              </tr>
              <tr>
                ...
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--cons" role="list">
                    <li v-for="(item, i) in comparison.platform.cons" :key="'p-c-' + i">{{ item }}</li>
                  </ul>
                </td>
              </tr>
            </tbody>
...
import { webappCalculatorConfig as calcConfig } from '~/config/webappCalculator'
...
const breadcrumbItems = computed(() => [
  ...
  { label: t('servicesTree.sites.sub.webapp') }
])

const content = computed(() => {
  const c = get('servicesTree.sites.webappContent')
  return c && typeof c === 'object' ? c : null
})

const comparison = computed(() => {
  const data = get('servicesTree.sites.webappComparison')
  return data?.custom && data?.platform ? data : null
})
...
const calcLabels = computed(() => get('servicesTree.sites.webappCalculator') || {})
