  ...
  <div class="page">
    <div class="page-inner section">
      ...
      <article class="article">
        ...
        <h1 class="article-title">{{ t('servicesTree.sites.sub.landing') }}</h1>
        ...
        <div class="comparison-table-wrap">
          <table class="comparison-table">
            <thead>
              <tr>
                <th scope="col"></th>
                <th scope="col">{{ comparison.landing.label }}</th>
                <th scope="col">{{ comparison.multipage.label }}</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--pros" role="list">
                    <li v-for="(item, i) in comparison.landing.pros" :key="'l-p-' + i">{{ item }}</li>
                  </ul>
                </td>
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--pros" role="list">
                    <li v-for="(item, i) in comparison.multipage.pros" :key="'m-p-' + i">{{ item }}</li>
                  </ul>
                </td>
              </tr>
              <tr>
                ...
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--cons" role="list">
                    <li v-for="(item, i) in comparison.landing.cons" :key="'l-c-' + i">{{ item }}</li>
                  </ul>
                </td>
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--cons" role="list">
                    <li v-for="(item, i) in comparison.multipage.cons" :key="'m-c-' + i">{{ item }}</li>
                  </ul>
                </td>
              </tr>
            </tbody>
...
import { landingCalculatorConfig as calcConfig } from '~/config/landingCalculator'
...
const breadcrumbItems = computed(() => [
  ...
  { label: t('servicesTree.sites.sub.landing') }
])

const content = computed(() => {
  const c = get('servicesTree.sites.landingContent')
  return c && typeof c === 'object' ? c : null
})

const comparison = computed(() => {
  const data = get('servicesTree.sites.landingComparison')
  return data?.landing && data?.multipage ? data : null
})
...
const calcLabels = computed(() => get('servicesTree.sites.landingCalculator') || {})
