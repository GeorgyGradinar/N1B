  ...
  <div class="page">
    <div class="page-inner section">
      ...
      <article class="article">
        ...
        <p class="article-category">{{ t('servicesTree.optimization.title') }}</p>
        <h1 class="article-title">{{ t('servicesTree.optimization.sub.audit') }}</h1>
        <div class="article-body">
          ...
          <p class="article-cta">{{ t('servicesTree.optimization.auditCta') }}</p>
          <p class="article-cta-sub">{{ t('servicesTree.optimization.auditCalcPromo') }}</p>
          <a href="#calculator" class="article-cta-btn">{{ t('servicesTree.optimization.auditCtaBtn') }}</a>
        </div>
      </article>
        ...
        <div class="comparison-table-wrap">
          <table class="comparison-table">
            <thead>
              <tr>
                <th scope="col"></th>
                <th scope="col">{{ comparison.express.label }}</th>
              ...
              <tr>
                ...
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--pros" role="list">
                    <li v-for="(item, i) in comparison.express.pros" :key="'e-p-' + i">{{ item }}</li>
                  </ul>
                </td>
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--pros" role="list">
              ...
              <tr>
                ...
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--cons" role="list">
                    <li v-for="(item, i) in comparison.express.cons" :key="'e-c-' + i">{{ item }}</li>
                  </ul>
                </td>
                <td>
                  <ul class="comparison-cell-list comparison-cell-list--cons" role="list">
...
import { auditCalculatorConfig as calcConfig } from '~/config/auditCalculator'
...
const breadcrumbItems = computed(() => [
  ...
  { label: t('servicesTree.optimization.title') },
  { label: t('servicesTree.optimization.sub.audit') }
])

const content = computed(() => {
  const c = get('servicesTree.optimization.auditContent')
  return c && typeof c === 'object' ? c : null
})

const comparison = computed(() => {
  const data = get('servicesTree.optimization.auditComparison')
  return data?.express && data?.full ? data : null
})
...
const calcLabels = computed(() => get('servicesTree.optimization.auditCalculator') || {})
...
const pageTitle = computed(() => t('servicesTree.optimization.sub.audit'))

useHead({
  title: () => `${pageTitle.value} - N1B`,
  meta: [{ name: 'description', content: () => t('servicesTree.optimization.shortDesc') }]
})
</script>
...
.article-cta-btn { display: inline-block; margin-top: 1rem; padding: 0.65rem 1.4rem; background: var(--color-accent, #ff6803); color: #fff; font-weight: 600; font-size: 0.95rem; border-radius: 8px; text-decoration: none; transition: opacity 0.15s; }
...
.comparison-cell-list--pros li::before { content: '✓'; position: absolute; left: 0; color: var(--color-accent, #ff6803); font-weight: 600; }
...
.calculator-option input { width: 1.1rem; height: 1.1rem; accent-color: var(--color-accent, #ff6803); }
...
.calculator-total-value { font-size: 1.75rem; font-weight: 700; margin: 0; color: var(--color-accent, #ff6803); }
