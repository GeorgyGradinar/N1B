  ...
  <div class="page">
    <div class="page-inner section">
      ...
      <article class="article">
        ...
        <p class="article-category">{{ t('servicesTree.apps.title') }}</p>
        <h1 class="article-title">{{ t('servicesTree.apps.sub.support') }}</h1>
...
import { supportCalculatorConfig as calcConfig } from '~/config/supportCalculator'
...
const breadcrumbItems = computed(() => [
  ...
  { label: t('servicesTree.apps.title') },
  { label: t('servicesTree.apps.sub.support') }
])

const content = computed(() => {
  const c = get('servicesTree.apps.supportContent')
  return c && typeof c === 'object' ? c : null
})

const comparison = computed(() => {
  const data = get('servicesTree.apps.supportComparison')
...
const calcLabels = computed(() => get('servicesTree.apps.supportCalculator') || {})
...
const total = computed(() => {
  ...
  return Math.max(Math.round(sum * multiplier), calcConfig.minPrice)
})

const totalFormatted = computed(() =>
  (calcLabels.value.totalPrefix ?? '') +
  new Intl.NumberFormat('ka-GE', { style: 'currency', currency: 'GEL', maximumFractionDigits: 0 }).format(total.value)
)

const pageTitle = computed(() => t('servicesTree.apps.sub.support'))
